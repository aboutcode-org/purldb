...
class SnippetIndexTestCase(MatchcodeTestCase):
    ...
    def test_SnippetIndexTestCase_match_resource(self):
        ...
        expected_fingerprints_loc = self.get_test_loc(
            "models/snippet-test-top-match-fingerprints.json"
        )
        self.check_expected_results(
            result_fingerprints, expected_fingerprints_loc, regen=False
        )
        assert match.fingerprints_count == 900
        assert match.similarity == 0.9202453987730062
