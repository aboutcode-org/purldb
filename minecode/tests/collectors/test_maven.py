...
class MavenPriorityQueueTests(JsonBasedTesting, DjangoTestCase):
    ...
    def test_map_maven_package(self):
        ...
        maven.map_maven_package(package_url, packagedb.models.PackageContentType.BINARY, ('test_pipeline'))
        package_count = packagedb.models.Package.objects.all().count()
    ...
    def test_get_merged_ancestor_package_from_maven_package(self, get_pom_text_mock, regen=FIXTURES_REGEN):
        ...
        with patch("minecode.collectors.maven.get_ancestry") as mock_get_ancestry:
            ...
            merged_package = maven.get_merged_ancestor_package_from_maven_package(package=db_package)
...
class MavenCrawlerFunctionsTest(JsonBasedTesting, DjangoTestCase):
    ...
    def test_check_if_file_name_is_linked_on_page(self):
        links = ['foo/', 'bar/', 'baz/']
        self.assertTrue(
            maven.check_if_file_name_is_linked_on_page('foo/', links)
        )
        self.assertFalse(
            maven.check_if_file_name_is_linked_on_page('qux/', links)
        )
    ...
    def test_check_on_page(self, mock_request_get):
        ...
        self.assertTrue(maven.check_on_page('https://repo1.maven.org/maven2/net/shibboleth/parent/7.11.0/', checker))
    ...
    def test_is_package_version_page(self, mock_request_get):
        ...
        '''
        self.assertTrue(maven.is_package_version_page('https://repo1.maven.org/maven2/xml-apis/xml-apis/1.0.b2/'))
    ...
    def test_create_url(self):
        ...
        self.assertEqual(
            url,
            maven.create_url(scheme, netloc, path_segments)
        )
    ...
    def test_determine_namespace_name_version_from_url(self, mock_request_get):
        ...
        namespace, package_name, package_version = maven.determine_namespace_name_version_from_url(url, root_url)
    ...
    def test_filter_only_directories(self):
        ...
        self.assertEqual(
            expected,
            maven.filter_only_directories(timestamps_by_links)
        )
    ...
    def test_collect_links_from_text(self):
        ...
        self.assertEqual(
            expected,
            maven.collect_links_from_text(text, filter=filter)
        )
    ...
    def test_create_absolute_urls_for_links(self):
        ...
        self.assertEqual(
            expected,
            maven.create_absolute_urls_for_links(text, url, filter=filter)
        )
    ...
    def test_get_artifact_sha1(self, mock_request_get):
        ...
        self.assertEqual(sha1, maven.get_artifact_sha1('https://repo1.maven.org/maven2/xml-apis/xml-apis/1.0.b2/xml-apis-1.0.b2.jar.sha1'))
