...
class GoLangPriorityQueueTests(JsonBasedTesting, DjangoTestCase):
    ...
    def test_extract_golang_subset_purl(self):
        test1 = "pkg:golang/github.com/rickar/cal/v2/aa@2.1.23"
        test2 = "pkg:golang/github.com/rickar/cal/v2/aa"
    ...
    def test_map_go_package_others(self):
        ...
        package_json = golang.scrape_go_package("golang.org/x/oauth2", "0.29.0")
        golang.map_golang_package(package_url, package_json, ("test_pipeline"))
        package_count = packagedb.models.Package.objects.all().count()
        self.assertEqual(1, package_count)
        package = packagedb.models.Package.objects.all().first()
    ...
    def test_scrape_go_package(self):
        metadata_dict = golang.scrape_go_package("golang.org/x/oauth2", "0.29.0")
        expected_dict = {
            "license_text": "BSD-3-Clause",
            "repository_homepage_url": "https://cs.opensource.google/go/x/oauth2",
            "download_url": "https://proxy.golang.org/golang.org/x/oauth2/@v/v0.29.0.zip",
        }
        self.assertEqual(expected_dict, metadata_dict)

    def test_scrape_package_versions(self):
        versions = golang.scrape_package_versions("golang.org/x/oauth2")
        # The version list may expand overtime, as of writing the test,
        # there are 29 releases
        expected_version_len = 29
        self.assertGreaterEqual(len(versions), expected_version_len)
