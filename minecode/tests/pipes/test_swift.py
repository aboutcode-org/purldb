...
import saneyaml
from minecode.pipes.swift import (
...
class SwiftPipelineTests(TestCase):
    def _run_package_test(self, package_repo_url, commits_tags_file, expected_file, regen=False):
        ...
        base_purl, generated_purls, _ = generate_package_urls(
        ...
        if regen:
            with open(expected_file, encoding="utf-8", mode="w") as f:
                results = {
                    "base_purl": str(base_purl),
                    "purls": sorted(generated_purls),
                }
                f.write(saneyaml.dump(results))

        with open(expected_file, encoding="utf-8") as f:
            expected = saneyaml.load(f)
        assert base_purl.to_string() == expected["base_purl"]
        assert sorted(generated_purls) == expected["purls"]
