...
class MavenMiscTest(JsonBasedTesting, DjangoTestCase):
    ...
    def test_get_entries(self):
        ...
        fields = (
            list(maven_visitor.ENTRY_FIELDS.keys())
            + list(maven_visitor.ENTRY_FIELDS_OTHER.keys())
            + list(maven_visitor.ENTRY_FIELDS_IGNORED.keys())
        )
        fields = set(fields)
    ...
    def test_get_artifacts_full(self):
        ...
        result = [
            a.to_dict()
            for a in maven_visitor.get_artifacts(index, fields, include_all=True)
        ]
    ...
    def test_get_artifacts_no_worthyness(self):
        ...
        result = [
            a.to_dict() for a in maven_visitor.get_artifacts(index, worthyness=worth)
        ]
        expected_loc = self.get_test_loc(
...
class MavenPriorityQueueTests(JsonBasedTesting, DjangoTestCase):
    ...
    def test_get_merged_ancestor_package_from_maven_package(
        ...
        with patch("minecode.visitors.maven.get_ancestry") as mock_get_ancestry:
            ...
            merged_package = (
                maven_visitor.get_merged_ancestor_package_from_maven_package(
                    package=db_package
                )
            )
            expected_loc = self.get_test_loc(
...
class MavenCrawlerFunctionsTest(JsonBasedTesting, DjangoTestCase):
    ...
    def test_is_package_version_page(self, mock_request_get):
        ...
        """
        self.assertTrue(
            maven_visitor.is_package_version_page(
    ...
    def test_create_url(self):
        ...
        url = "https://example.com/foo/bar/baz.jar"
        self.assertEqual(url, maven_visitor.create_url(scheme, netloc, path_segments))
    ...
    def test_determine_namespace_name_version_from_url(self, mock_request_get):
        ...
        namespace, package_name, package_version = (
            maven_visitor.determine_namespace_name_version_from_url(url, root_url)
        )
    ...
    def test_filter_only_directories(self):
        ...
        self.assertEqual(
            expected, maven_visitor.filter_only_directories(timestamps_by_links)
        )
    ...
    def test_collect_links_from_text(self):
        ...
        self.assertEqual(
            expected, maven_visitor.collect_links_from_text(text, filter=filter)
        )
    ...
    def test_get_artifact_sha1(self, mock_request_get):
        ...
        self.assertEqual(
            sha1,
            maven_visitor.get_artifact_sha1(
