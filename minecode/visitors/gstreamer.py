...
@visit_router.route('https://gstreamer.freedesktop.org/src/([\w\-\.]+/)*')
            ...
            if href:
                ...
                if href.endswith('/'):
                    ...
                    if '-' in file_name_without_prefix:
                        ...
                        version = None
                    package_url = PackageURL(type='gstreamer', name=project_name, version=version).to_string()
                    yield URI(uri=url_template.format(sub_path=href), package_url=package_url, file_name=file_name, source_uri=self.uri)
