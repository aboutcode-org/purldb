...
@map_router.route(r"http://ftp.debian.org/debian/pool/.*\.dsc")
...
def parse_description(metadata, purl=None, base_download_url=None):
    ...
    collect_files(
        existing_files=files,
        field_value=metadata.get("Checksums-Sha256"),
    ...
    for source in debcon.get_paragraphs_data(metadata):
        ...
        for f in files:
            ...
            download_url = "http://ftp.debian.org/debian/{path}/{name}".format(
                path=source.get("Directory"), name=name
            )
...
def parse_packages(metadata, purl=None):
    ...
    for pack in debcon.get_paragraphs_data(metadata):
        ...
        if filename:
            data["download_url"] = f"http://ftp.debian.org/debian/{filename}"
...
@map_router.route(
    r"http://ftp.debian.org/debian/dists/.*\.zip",
    r"http://ftp.debian.org/debian/dists/.*\.jar",
    r"http://ftp.debian.org/debian/dists/.*\.gz",
)
