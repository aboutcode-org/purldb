...
class Coordinate:
    ...
    def to_harvest_blob_path(self, tool, tool_version):
        return "{type}/{provider}/{namespace}/{name}/revision/{revision}/tool/{tool}/{tool_version}.json".format(
            tool=tool, tool_version=tool_version, **self.to_dict()
        )
    ...
    def to_def_query_api_url(self, include_revision=False, base_api_url=None):
        ...
        qs = qs.format(base_url=base_api_url or self.base_api_url, **self.to_dict())
...
def coord2str(coord):
    ...
    """
    assert coord, f"Empty or missing coordinate mapping: {coord}"
